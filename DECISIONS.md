...
## Workbench expansion (Excel analysis companion)

- **`xlsx-js-style` over plain `xlsx` for the generator.** The starter workbook needs header fills + bold, which the community SheetJS build won't write. `xlsx-js-style` (a styling fork of SheetJS 0.18) writes fills/fonts/borders/column-widths. The known SheetJS parsing advisory doesn't apply — the app only WRITES files, never parses untrusted ones. Verified the output in Node: 6 tabs, valid Content_Types/styles, correctly-escaped SUMIFS structured-reference formulas, header fills attach (cells carry `s=` indices).
- **SUMIFS, not native PivotTables, in the .xlsx.** SheetJS can't author pivot caches, so 2-Summary is a SUMIFS-driven top-codes table that recalcs once the user names their table `tblCharges`. The six native-pivot recipes live in the app instead. Noted in the READ-ME tab.
- **Freeze panes dropped from the generator.** Confirmed unsupported by the xlsx-js-style writer (no `<pane>` emitted). Rather than ship a misleading no-op, the READ-ME tells the user to View → Freeze Top Row. Column widths and styling (which DO work) are kept.
- **Bulk-paste parser is delimiter-aware, not naive CSV.** Tab-separated is preferred (Excel copy); falls back to comma. Numbers are cleaned of `$`/commas after splitting, so "$980,000" survives. Column role (qty vs charges) is inferred by `$` presence then by magnitude (charges ≫ qty); genuinely ambiguous input surfaces a two-button picker. Capped at 50 rows (shortlist tool, not ingestion). Verified the heuristics in Node and the live preview/EXCLUDED-badge path in the browser.
- **Dedupe by code (the app's existing model), not code+service line.** `quickAdd` keys on code; bulk commit updates figures on an existing entry and never overwrites its research, matching the rest of the app. A second service line for the same code isn't representable in the current data model, so I kept the established behavior rather than fork it.
- **Research queue is sequential and rate-limit-aware.** Honors the 20/hour gate; on a 429 it stops and surfaces the friendly message with the count completed, rather than hammering the API.
- **`nowMs()` helper added to utils.** React-compiler's purity lint flags the literal `Date.now` global inside component scope (inconsistently). Routing it through an imported helper — as `makeId` already does — keeps component code lint-clean without disabling the rule.
- **9th nav slot.** Workbench sits between Library and Verify (daily-loop placement); the mobile tab bar went from 8 to 9 columns.
